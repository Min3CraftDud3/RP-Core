    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(cmd.getName().equalsIgnoreCase("perm") && sender.hasPermission("Perm.Command")){
            if(args.length==0){
                ...
                sender.sendMessage("Add/Remove Player Permissions:"+ ChatColor.GREEN+" /perm <player> add/remove <permission>");
                sender.sendMessage("Add/Remove Group Permissions:"+ ChatColor.GREEN+" /perm <group> add/remove <permission>");
