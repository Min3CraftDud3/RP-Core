...
public class MonsterFile {
    ...
    public MonsterFile(RPCore plugin) {
        this.plugin = plugin;
    }
    ...
    public static int findNearest(double xCoord, double zCoord){ /*Possible Method for Traversing the file*/
        File mobFile = new File(plugin.getDataFolder() + File.separator + "data" + File.separator + "Mobs.yml");
        int ID = 0, RID = -1;
        ...
        if (mobFile.exists()) {
            FileConfiguration fc = YamlConfiguration.loadConfiguration(mobFile);
            try {
                for (String id : fc.getConfigurationSection("Mobs").getKeys(false)) {
    ...
    public static int getLastID() {/*The Method I will use to generate a quick and easy ID*/
        File mobFile = new File(plugin.getDataFolder() + File.separator + "data" + File.separator + "Mobs.yml");
        FileConfiguration fc = YamlConfiguration.loadConfiguration(mobFile);
        if (mobFile.exists()) {
            Set<String> ids = fc.getConfigurationSection("Mobs").getKeys(false);
